...
namespace UniTimetable
{
    ...
    {
        TimeOfWeek ClickTime_ = null;
        Session ClickSession_ = null;
        Unavailability ClickUnavail_ = null;
        ...
        FormImport FormImport_ = new FormImport();
        ...
        private void UpdateToolstrip()
        {
            ...
            /*TextImageRelation layout = (large ? TextImageRelation.ImageAboveText : TextImageRelation.ImageBeforeText);
            btnNew.TextImageRelation = layout;
            btnImport.TextImageRelation = layout;
            btnOpen.TextImageRelation = layout;
            btnSave.TextImageRelation = layout;
            btnPrint.TextImageRelation = layout;
            btnCriteria.TextImageRelation = layout;
            btnSolver.TextImageRelation = layout;
            btnLucky.TextImageRelation = layout;
            btnAddByClass.TextImageRelation = layout;
            btnClear.TextImageRelation = layout;*/
        ...
        {
            if (ClickSession_ == null)
                return;
            ClickSession_.Stream.Selected = false;
            MadeChanges(true);
        }

        private void unavailabilityToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (FormUnavail_.ShowDialog(Timetable_, new Timeslot(ClickTime_.Day, ClickTime_.Hour, 0, ClickTime_.Hour + 1, 0), timetableControl1.HourStart, timetableControl1.HourEnd) == DialogResult.Cancel)
                return;
            MadeChanges(true);
        }
